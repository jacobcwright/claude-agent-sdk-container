...
const SECRET = new TextEncoder().encode(
  process.env.SESSION_SECRET || 'devdevdev'
)
...
const allowedGithubUsers =
  process.env.ALLOWED_GITHUB_USERS?.split(',').map((u) =>
    u.trim().toLowerCase()
  ) || []
const allowedGithubOrg =
  process.env.ALLOWED_GITHUB_ORG?.trim().toLowerCase() || ''

// Check if running in Docker/Kubernetes container (skip check in test mode)
if (process.env.NODE_ENV !== 'test') {
  // Check for Docker
  const hasDockerenv = fs.existsSync('/.dockerenv')
  const hasDockerContainer = process.env.container === 'docker'

  // Check for Kubernetes (common indicators)
  const hasKubernetesService = !!process.env.KUBERNETES_SERVICE_HOST
  const hasKubernetesPort = !!process.env.KUBERNETES_SERVICE_PORT

  // Check cgroup for container indicators (Docker/Kubernetes)
  let hasContainerCgroup = false
  try {
  ...
  const isContainerized =
    hasDockerenv ||
    hasDockerContainer ||
    hasKubernetesService ||
    hasKubernetesPort ||
    hasContainerCgroup

  if (!isContainerized && process.env.ALLOW_LOCAL !== 'true') {
    console.error(
      '\n❌ ERROR: This application must be run in a containerized environment!'
    )
    console.error('\n📋 Quick Start:')
    console.error('1. Ensure Docker is running')
    console.error('2. Run the test script: ./test.sh')
    console.error(
      '   (The script handles Docker build, run, and testing automatically)\n'
    )
    console.error(
      'To bypass this check (not recommended): ALLOW_LOCAL=true tsx server.ts\n'
    )
    process.exit(1)
  }
  ...
  if (allowedGithubUsers.length === 0 && !allowedGithubOrg) {
    ...
    console.error(
      'You must configure at least one of these environment variables:'
    )
...
if (process.env.NODE_ENV !== 'test') {
  const isDocker =
    fs.existsSync('/.dockerenv') || process.env.container === 'docker'
  ...
  console.log(
    'API protection:',
    !!process.env.CLAUDE_AGENT_SDK_CONTAINER_API_KEY ? '✓' : '✗'
  )
  console.log(
    'GitHub OAuth:',
  ...
  if (allowedGithubUsers.length > 0)
    console.log('GitHub users allowlist:', allowedGithubUsers.length, 'users')
...
app.post('/query', async (c) => {
  try {
    ...
    if (prompt.length > 100000) {
      return c.json(
        { error: 'Prompt too long. Maximum 100000 characters' },
        400
      )
    }
    ...
    if (
      ...
      return c.json(
        {
          error:
            'ANTHROPIC_API_KEY or CLAUDE_CODE_OAUTH_TOKEN must be configured',
        },
        401
      )
    }
    ...
    const queryOptions: any = {
      ...
      agents: {
        canadian_agent: {
          description:
            "Provides a friendly Canadian perspective on the user's request",
...
Keep your responses concise (2-3 sentences) and always make it clear you're the Canadian perspective.`,
        ...
        australian_agent: {
          description:
            "Provides a laid-back Australian perspective on the user's request",
